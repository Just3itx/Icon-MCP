...
const ALL_HARNESSES = [
  ...
  { id: "zed", name: "Zed", group: "Others", config: { kind: "zedJson", path: homePath(".config", "zed", "settings.json") } },
  { id: "pearai", name: "PearAI", group: "Others", config: { kind: "mcpServersJson", path: homePath(".pearai", "mcp.json"), experimental: true } },
  { id: "void", name: "Void Editor", group: "Others", config: { kind: "mcpServersJson", path: homePath(".void", "mcp.json"), experimental: true } },
  { id: "aider", name: "Aider", group: "Others", config: { kind: "aiderConf", path: homePath(".aider.conf.yml"), experimental: true } },
  { id: "codeium-cli", name: "Codeium CLI", group: "Others", config: { kind: "mcpServersJson", path: homePath(".codeium", "mcp.json"), experimental: true } },
  { id: "amazon-q", name: "Amazon Q", group: "Others", config: { kind: "mcpServersJson", path: homePath(".aws", "q", "mcp.json"), experimental: true } },
  { id: "sourcegraph-cody", name: "Sourcegraph Cody", group: "Others", config: { kind: "mcpServersJson", path: homePath(".config", "cody", "mcp.json"), experimental: true } },
  { id: "jetbrains-gateway", name: "JetBrains Gateway", group: "Others", config: { kind: "continueYaml", path: homePath(".config", "JetBrains", "Gateway", "config.yaml"), experimental: true } },
...
async function main() {
  ...
  console.log(`\n${colors.green}Done.${colors.reset} Restart selected clients to use Icon MCP.`);
  showCursor();
}
...
async function installServer(serverRoot, results, options = {}) {
  ...
  const runner = exists(path.join(serverRoot, "pnpm-lock.yaml")) ? "pnpm" : exists(path.join(serverRoot, "yarn.lock")) ? "yarn" : "npm";
  await run(
    runner,
  ...
  try {
    switch (harness.config.kind) {
      ...
      case "vibeToml":
        ...
        break;
      case "zedJson":
        await writeZedJson(harness.config.path, serverEntry);
        break;
      case "aiderConf":
        await writeAiderConf(harness.config.path, serverEntry);
        break;
      case "claudeCli":
...
async function configureClaudeCode(serverEntry) {
  if (!commandExists("claude")) {
    throw new Error("Claude Code CLI not found. Run manually: claude mcp add --global icon-mcp -- node " + quote(serverEntry));
  }
...
async function writeAmpJson(filePath, serverEntry) {
  await mergeJsoncKey(filePath, "amp.mcpServers", SERVER_NAME, mcpServerConfig(serverEntry));
}
...
async function writeContinueYaml(filePath, serverEntry) {
  const markerStart = "# icon-mcp:start";
  const markerEnd = "# icon-mcp:end";
...
async function writeZedJson(filePath, serverEntry) {
  await mergeJsoncKey(filePath, "context_servers", SERVER_NAME, mcpServerConfig(serverEntry));
}

async function writeAiderConf(filePath, serverEntry) {
  const markerStart = "# icon-mcp:start";
  const markerEnd = "# icon-mcp:end";
  const block = `${markerStart}\nmcp-servers:\n  - node ${yamlString(serverEntry)}\n${markerEnd}\n`;
  let text = exists(filePath) ? await fs.readFile(filePath, "utf8") : "";
  const re = new RegExp(`${escapeRe(markerStart)}[\\s\\S]*?${escapeRe(markerEnd)}\\n?`, "m");
  if (re.test(text)) text = text.replace(re, block);
  else text = `${text.trimEnd()}${text.trimEnd() ? "\n\n" : ""}${block}`;
...
async function selectHarnesses(initial) {
  ...
  return await new Promise((resolve) => {
    ...
    const render = () => {
      ...
      printBanner();
      console.log(`${colors.green}${ASCII_MODE ? ">" : "◆"}${colors.reset} Which harnesses do you want to install Icon MCP into?\n`);
...
async function selectHarnessesPlain(initial) {
  ...
  console.log(`${colors.cyan}${colors.bold}Icon MCP Setup${colors.reset}`);
...
async function mergeJsoncKey(filePath, parentKey, childKey, childValue) {
  if (!exists(filePath)) {
    const json = {};
    json[parentKey] = { [childKey]: childValue };
    await writeJson(filePath, json);
    return;
  }

  let text = await fs.readFile(filePath, "utf8");
  const childPayload = JSON.stringify(childValue, null, 2);
  const indentedPayload = childPayload.replace(/\n/g, "\n    ");
  const entryStr = `"${childKey}": ${indentedPayload}`;

  const escapedParent = parentKey.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
  const escapedChild = childKey.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");

  const childRe = new RegExp(`"${escapedChild}"\\s*:\\s*\\{[^}]*\\}`);
  const parentRe = new RegExp(`("${escapedParent}"\\s*:\\s*\\{)`);

  if (parentRe.test(text)) {
    if (childRe.test(text)) {
      text = text.replace(childRe, entryStr);
    } else {
      text = text.replace(parentRe, `$1\n    ${entryStr},`);
    }
  } else {
    const parentBlock = `"${parentKey}": {\n    ${entryStr}\n  }`;
